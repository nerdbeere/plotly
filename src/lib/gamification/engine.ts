...
export async function completeTaskWithReward(taskId: number) {
  ...
  const scheduledNext = scheduleNextOccurrence({ userPlantId: task.userPlantId, taskType: task.taskType }, now) || scheduleNextAreaOccurrence({ gardenAreaId: task.gardenAreaId, taskType: task.taskType }, now);
